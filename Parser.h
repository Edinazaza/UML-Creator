...
public:
	...
	void output(const std::string& filepath = "nofile.txt") const
	{
		...
		{
			first = true;
			(filepath == "nofile.txt" ? std::cout : stream) << '\n' << sections_to_chars(item.first.second) << " " << item.first.first.second << '(';
			for (const auto& itm : item.second)
			{
				if (!first)
					(filepath == "nofile.txt" ? std::cout : stream) << ", ";
				(filepath == "nofile.txt" ? std::cout : stream) << "in " << itm.first << ": " << itm.second;
				first = false;
			}
